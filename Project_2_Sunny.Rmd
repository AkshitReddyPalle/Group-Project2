...
```
The coefficients for the two variables are higher than 0.8. Drop `Percent_pop_under_1.37_of_poverty_threshold` and run the model 
```{r}
...
```
The coefficients for `Percent_pop_American_Indian_and_Alaska_Native` and `Percent_pop_Black_and_African_American ` are positively significant, meaning that counties with higher percentage of American Indian and Black have higher maternal mortality rate.

The coefficient for `Median_household_income` is negatively significant, indicating that Higher median household income is associated with a lower maternal mortality rate.


The coefficient for `Number_of_hospitals_with_ICU` is negatively significant, suggesting that More hospitals with ICUs are associated with a lower maternal mortality rate.

The coefficient for `Percent_pop_30_44` is negatively significant, indicating a higher percentage of women aged 30-44 is associated with a lower maternal mortality rate. It may be explained that compared with the base group (older than 44), this group has lower maternal mortality rate.

The coefficient for `Number_of_hospitals_with_obstetric` is positively significant which might reflect referral bias, where areas with higher maternal mortality attract more obstetric services.
...
features <- c("Percent_pop_Black_and_African_American","Percent_pop_30_44",'Median_distance_to_nearest_obstetrics_department','Number_of_hospitals_with_obstetric','Median_household_income','Percent_femal_pop')
...
The first, second, fourth, and fifth partial dependence plots align with the conclusions drawn from the linear regression analysis.

The partial dependence plot for Median_distance_to_nearest_obstetrics_department shows an initial increase followed by a slight decrease as the Median_distance_to_nearest_obstetrics_department increases. This suggests that, in most counties, maternal mortality rates tend to rise with increasing distances to obstetrics departments, indicating that access to obstetric care is a critical factor. However, in rural counties, where the Median_distance_to_nearest_obstetrics_department is typically higher, the population density is lower, resulting in fewer pregnancies and, consequently, a lower maternal mortality rate despite the greater distances.
